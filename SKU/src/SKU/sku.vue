...
import powerSet from '@/SKU/power-set.js'
// 商品数据
const goods = ref({})
let pathMap = {}
...
const changeSelectedStatus = (item, val) => {
    ...
    if(index > -1){
        ...
        console.log(SkuIds)
        //以skuid作为匹配项，去goods.values.skus里进行寻找 拿到对应的对象
        const skuObj = goods.value.skus.find(item => item.id === SkuIds[0])
        console.log(skuObj)
        handleSelect(skuObj)
    }
}
const emit = defineEmits(['sendSkuInfo'])
const handleSelect = (selection) => {
    const skuInfo = {selection};
    emit('sendSkuInfo',skuInfo)
}
...
const getSelectedValues = (specs) => {
    const arr = []
    specs.forEach(spec => {
        //找到values中selected中是true的项，然后把他的name字段添加数组对应的位置
            ...
            <dd>
                <template v-for="val in item.values" :key="val.name">
                    ...
                    <img :class="{ selected: val.selected, disabled: val.disabled }"
                        @click="changeSelectedStatus(item, val)" v-if="val.picture" :src="val.picture"
                        :title="val.name">
                    <!-- 文字类型规格 -->
